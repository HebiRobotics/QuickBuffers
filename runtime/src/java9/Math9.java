...
public class Math9 {
    ...
    public static long multiplyHigh(long x, long y) {
        if (USE_JDK) {
            ...
            return multiplyHigh_fallback(x, y);
        }
    }

    private static long multiplyHigh_fallback(long x, long y) {
        if (x < 0 || y < 0) {
    ...
    private static final boolean USE_JDK;

    static {
        boolean hasMultiplyHigh;
        try {
            java.lang.Math.multiplyHigh(0, 0);
            hasMultiplyHigh = true;
        } catch (NoSuchMethodError oldRuntime) {
            hasMultiplyHigh = false;
        }
        USE_JDK = hasMultiplyHigh;
    }
